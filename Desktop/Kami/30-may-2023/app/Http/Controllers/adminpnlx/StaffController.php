...
use Illuminate\Support\Facades\{DB, Session, Validator, Redirect, Auth, Crypt, Hash, Str, Notification};
...
class StaffController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->all()) {
            ...
            foreach ($searchData as $fieldName => $fieldValue) {
                if ($fieldName == "name") {
                    $DB->where(DB::raw("CONCAT(`first_name`, ' ', `last_name`)"), 'LIKE', "%".$fieldValue."%");
                }
                if ($fieldName == "email") {
                ...
                if ($fieldName == "is_active") {
                    $DB->where("admins.is_active", 'LIKE', '%' . $fieldValue . '%');
                }
                $searchVariable = array_merge($searchVariable, array($fieldName => $fieldValue));
            }
        }
        $DB->where('admins.is_deleted', 0);
        $DB->where('admins.user_role_id', 2);
        $sortBy = ($request->input('sortBy')) ? $request->input('sortBy') : 'admins.created_at';
        $order  = ($request->input('order')) ? $request->input('order')   : 'DESC';
        $records_per_page  =   ($request->input('per_page')) ? $request->input('per_page') : Config("Reading.records_per_page");
        $results = $DB->orderBy($sortBy, $order)->paginate($records_per_page);
        Session::put('export_data_admin', $results);
    ...
    public function create(){
        ...
        return view('adminpnlx.' . $this->modelName . '.add', compact('departments'));
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()->back()->withErrors($validator)->withInput();
                ...
                $obj                            = new Admin;
                $obj->user_role_id              = Config('constants.ROLE_ID.ADMIN_ROLE_ID') ;
            ...
            $obj->save();
            Session::flash('success', trans(Config('constants.STAFFS.STAFF_TITLE') . " has been Added successfully"));
    ...
    public function edit($id)
    {
        ...
        if ($modelId) {
            $modelDetails = Admin::where('id', $modelId)->first();
            $departments  = Department::where('is_active', 1)->get();
            return view("adminpnlx." . $this->modelName . ".edit", compact('modelDetails', 'departments'));
        } else {
            return Redirect::back();
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!empty($formData)) {
            ...
            if ($validator->fails()) {
                ...
                $obj->save();
                Session::flash('success', trans(Config('constants.STAFFS.STAFF_TITLE') . " has been updated successfully"));
                return Redirect::route($this->modelName . ".index");
    ...
    public function destroy($id)
    {
        ...
        if ($modelId) {
            ...
            $statusMessage   =   trans(Config('constants.STAFFS.STAFF_TITLE') . " has been deleted successfully");
    ...
    public function deleteImage($id)
    {
        $modelId = base64_decode($id);
        if ($modelId) {
           $modelImage =  Admin::where('id', $modelId)->first();
            $modelImage->image = null;
            $modelImage->save();
            $statusMessage   =   trans(Config('constants.STAFFS.STAFF_TITLE') . " has been image deleted successfully");
            Session()->flash('flash_notice', $statusMessage);
        }
        return redirect::back();
    }
    ...
    public function changeStatus($modelId = 0, $status)
    {
        $modelId = base64_decode($modelId);
        if ($status == 1) {
            $statusMessage   =   trans(Config('constants.STAFFS.STAFF_TITLE') ." has been deactivated successfully");
        } else {
            $statusMessage   =   trans(Config('constants.STAFFS.STAFF_TITLE') ." has been activated successfully");

        }
        $user = Admin::findOrfail($modelId);
        if ($user) {
    ...
    public function designation(Request $request){
        $modelId = $request->input('dept_id');
        $designations = Designation::where('department_id', $modelId)->get();
        $designation_data = view('adminpnlx.' . $this->modelName . '.designation', compact('designations'))->render();
    ...
    public function permission(Request $request){
        $modelId = $request->input('desig_id');
        $module = Module::where('department_id', $modelId)->get();
        $module = view('adminpnlx.' . $this->modelName . '.staff_permission_data', compact('module'))->render();
    ...
    public function changedPassword(Request $request, $enuserid = null)
    {
        ...
        if ($request->isMethod('POST')) {
            if (!empty($modelId)) {
                ...
                $userDetails   =  Admin::find($modelId);
        ...
        $modelDetails   =  Admin::find($modelId);
        // dd(Auth::guard('admins')->user()->getAuthPassword());
    ...
    public function sendCredential(Request $request, $enuserid = null){
        ...
        if (!empty($enuserid)) {
            ...
            return redirect()->route($this->modelName . ".index");
             ...
             $userDetails           =  Admin::where('id', $modelId)->first();
                ...
                Session::flash('success', trans("Login credentials send successfully")); 
                return redirect()->back();
    }
    ...
    public function staffExport(Request $request)
    {
        ...
        $customers_export = Session::get('export_data_admin');
        if(empty($customers_export)){
            $table      = Admin::select('users.*')->get();
