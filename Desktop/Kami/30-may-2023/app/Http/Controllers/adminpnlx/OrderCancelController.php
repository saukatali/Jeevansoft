...
class OrderCancelController extends Controller
{
    ...
    {  
        ...
        if ($modelId) {
            $modelDetails = OrderCancel::where('id', $modelId)->first();
    ...
    public function destroy($id)
    {
        ...
        if ($modelId) {
            OrderCancel::where('id', $modelId)->delete();
            $statusMessage   =  trans(Config('constants.ORDER_CANCELS.ORDER_CANCEL_TITLE') ." has been deleted successfully");
    ...
    public function changeStatus($modelId = 0, $status)
    {
        $modelId = base64_decode($modelId);
        if ($status == 1) {
            $statusMessage   =   trans(Config('constants.ORDER_CANCELS.ORDER_CANCEL_TITLE') ." has been deactivated successfully");
        } else {
            $statusMessage   =   trans(Config('constants.ORDER_CANCELS.ORDER_CANCEL_TITLE') ." has been activated successfully");
        }
        $user = OrderCancel::findOrfail($modelId);
        if ($user) {
