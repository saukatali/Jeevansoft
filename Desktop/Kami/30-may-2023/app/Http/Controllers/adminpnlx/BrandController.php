...
class BrandController extends Controller
{
        ...
        if ($request->all()) {
            ...
            foreach ($searchData as $fieldName => $fieldValue) {
                ...
                if ($fieldName == "is_active") {
                    $DB->where("brands.is_active", 'LIKE', '%' . $fieldValue . '%');
                }
    ...
    {        
        ...
        if ($validator->fails()) {
            ...
            $obj->save();
            Session::flash('success', trans(Config('constants.BRANDS.BRAND_TITLE') . " has been Added successfully"));
    ...
    {  
        ...
        if ($modelId) {
            $modelDetails = Brand::where('id', $modelId)->first();
    ...
    public function edit($id)
    {
        ...
        if ($modelId) {
            $model = Brand::where('id', $modelId)->first();
    ...
    public function update(Request $request, $id)
    {
        $formData               = $request->all();
        $modelId                = base64_decode($id);
        $model                  = Brand::findorFail($modelId);
        ...
        if (!empty($formData)) {           
            ...
            if ($validator->fails()) {
                ...
                $obj->save();
                Session::flash('success', trans(Config('constants.BRANDS.BRAND_TITLE') . " has been updated successfully"));
                return Redirect::route($this->modelName . ".index");
    ...
    public function delete($id)
    {
        ...
        $modelId = base64_decode($id);
        if ($modelId) {
            Brand::where('id', $modelId)->delete();
            $statusMessage   =  trans(Config('constants.BRANDS.BRAND_TITLE') ." has been deleted successfully");
    ...
    public function changeStatus($modelId = 0, $status)
    {
        $modelId = base64_decode($modelId);
        if ($status == 1) {
            $statusMessage   =   trans(Config('constants.BRANDS.BRAND_TITLE') ." has been deactivated successfully");
        } else {
            $statusMessage   =  trans(Config('constants.BRANDS.BRAND_TITLE') ." has been activated successfully");
        }
        $user = Brand::findOrfail($modelId);
        if ($user) {
    ...
    {       
        ...
        if ($modelId) {
           $modelImage =  Color::where('id', $modelId)->first();
            ...
            $modelImage->save();
            $statusMessage   = trans(Config('constants.BRANDS.BRAND_TITLE') ."image has been deleted successfully");
            Session()->flash('flash_notice', $statusMessage);
        }
        return redirect::back();
    }
